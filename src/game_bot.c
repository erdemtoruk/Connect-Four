...
int best_move(TreeNode* root){
    ...
    for (int i = 0; i < root->num_children; i++){
        if(root->children[i]->game_state->evaluation == root->game_state->evaluation){
            return i;
        }
    }
    return 0;
...
int get_max(TreeNode* node){
    ...
    for(int i = 0; i < node->num_children; i++){
        ...
        new_score = get_min(node->children[i]);
        if(new_score > high_score){
            high_score = new_score;
        }
    }

    node->game_state->evaluation = high_score;
    return high_score;
}
...
int get_min(TreeNode* node){
    ...
    int least_score = 1000;
    int new_score = 0;

    for(int i = 0; i < node->num_children; i++){

        new_score = get_max(node->children[i]);
        if(new_score < least_score){
            least_score = new_score;
        }
    }

    node->game_state->evaluation = least_score;
    return least_score;
}
